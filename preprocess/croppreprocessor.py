...
class CropPreProcessor:
    def __init__(self, width, height, horiz=True, inter=cv2.INTER_AREA):
        # store the target image width, height, whether or not horizontal flips should be included,
        # along with the interpolation method used when resizing
        self.width = width
        self.height = height
        self.horiz = horiz
        self.inter = inter
    ...
    def preprocess(self, image):
        ...
        for (startX, startY, endX, endY) in self.coords:
            ...
            crop = cv2.resize(image, (self.width, self.height), interpolation=self.inter)
            crops.append(crop)
